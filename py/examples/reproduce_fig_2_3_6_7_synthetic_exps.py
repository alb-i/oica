...
def figure2(outdir: str, *, quick: bool) -> None:
    p = 10
    ks = [5, 10, 20, 30, 40, 50, 60] if quick else list(range(5, 61, 5))
    nrep = 3 if quick else 10
    algs = filter_algs(["rand", "foobi", "oica-semiada"])

    print(f"\n[Figure 2] experiment_asymptotic  p={p} ks={ks} nrep={nrep}")
    res = experiment_asymptotic(
...
def figure3_short(outdir: str, *, quick: bool) -> None:
    p = 15
    k = 30
    ns = [1000, 3000, 5000, 7000, 10000] if quick else list(range(1000, 10001, 1000))
    nrep = 3 if quick else 10
    algs = filter_algs(["foobi", "oica", "fpca", "rand"])

    print(f"\n[Figure 3 short] experiment_fixedk  p={p} k={k} ns={ns} nrep={nrep}")
    res = experiment_fixedk(
        p, k, nrep, algs, ns,
        expdir=os.path.join(outdir, "fixk30p15_short"),
        rng=np.random.default_rng(0),
    )
...
def figure3_and_7(outdir: str, *, quick: bool) -> None:
    ...
    ns = ([10_000, 50_000, 100_000, 200_000] if quick
          else list(range(10_000, 200_001, 10_000)))
    nrep = 3 if quick else 10
    algs = filter_algs(["foobi", "oica", "fpca", "oica-quad-semiada"])

    print(f"\n[Figures 3 & 7] experiment_fixedk  p={p} k={k} ns={ns} nrep={nrep}")
    res = experiment_fixedk(
        p, k, nrep, algs, ns,
        expdir=os.path.join(outdir, "fixk30p15"),
        rng=np.random.default_rng(0),
    )
...
def figure_runtime(outdir: str, *, quick: bool) -> None:
    p = 20
    ks = [20, 40, 60, 80, 100] if quick else list(range(20, 101, 20))
    n = 20_000 if quick else 100_000
    nrep = 2 if quick else 10
    algs = filter_algs(["foobi", "fpca", "oica"])

    print(f"\n[Figure 4 runtime] experiment_fixedn  p={p} ks={ks} n={n} nrep={nrep}")
    res = experiment_fixedn(
        p, ks, nrep, algs, n,
...
def main() -> None:
    ...
    if "fig2" in selected:
        figure2(args.outdir, quick=quick)
    if "fig3_short" in selected:
        figure3_short(args.outdir, quick=quick)
    if "fig3_fig7" in selected:
        figure3_and_7(args.outdir, quick=quick)
    if "runtime" in selected:
        figure_runtime(args.outdir, quick=quick)

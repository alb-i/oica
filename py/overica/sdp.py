...
``# MATLAB: ...`` comments inside each body map each Python statement back
to the MATLAB line(s) it implements and add short notes on what is being
done.
"""
...
def proj_simplex(v: np.ndarray) -> np.ndarray:
    ...
    v = np.where(v > 0, v, 0.0)                              # MATLAB: v = (v > 0) .* v
    u = np.sort(v)[::-1]                                     # MATLAB: u = sort(v,'descend')
    sv = np.cumsum(u)                                        # MATLAB: sv = cumsum(u)
    idx = np.arange(1, len(u) + 1)                           # MATLAB: 1:length(u)
    cond = u > (sv - 1.0) / idx                              # MATLAB: u > (sv - 1) ./ (1:length(u))'
    if not np.any(cond):                                     # (defensive: all-zero input)
        return np.zeros_like(v)
    rho = np.max(np.flatnonzero(cond)) + 1                   # MATLAB: rho = find(..., 1, 'last')
    theta = max(0.0, (sv[rho - 1] - 1.0) / rho)              # MATLAB: theta = max(0, (sv(rho)-1)/rho)
    return np.maximum(v - theta, 0.0)                        # MATLAB: w = max(v - theta, 0)
...
def extract_largest_eigenvector(D: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    D = (D + D.T) / 2.0                                      # MATLAB: D = (D+D')/2  (symmetrise)
    w, V = np.linalg.eigh(D)                                 # MATLAB: [u,e] = eig(D)  (eigh = symmetric eig)
    idx = int(np.argmax(np.abs(w)))                          # MATLAB: [a,b] = max(abs(diag(e)))
    u = np.real(V[:, idx])                                   # MATLAB: u = real(u(:,b(1)))
    return u, float(np.abs(w[idx]))                          # MATLAB: e = a(1)   (magnitude)
...
def extract_basis(Es: np.ndarray, k: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    """
    Q, _ = np.linalg.qr(Es, mode="complete")                 # MATLAB: [Q,~] = qr(Es)   (full QR)
    return Q[:, :k], Q[:, k:]                                # MATLAB: Esbasis=Q(:,1:k); Fsbasis=Q(:,k+1:end)
...
def solve_relaxation_mezcal_approx_fista(
    ...
    """
    p2, _ = Fsbasis.shape
    p = int(round(np.sqrt(p2)))                              # MATLAB: d = sqrt(d)  (reuses ``d`` for p)

    D = Dinit.copy()                                         # MATLAB: D = Dinit
    E = D.copy()                                             # MATLAB: E = D
    L = mu                                                   # MATLAB: L = mu  (Lipschitz constant)
    t = 1.0                                                  # MATLAB: t = 1   (FISTA momentum)

    primal_vals = np.zeros(maxiter)                          # MATLAB: primal_vals = zeros(1,maxiter)
    dual_vals = np.zeros(maxiter)                            # MATLAB: dual_vals = zeros(1,maxiter)

    for it in range(maxiter):                                # MATLAB: for iter = 1:maxiter
        temp = Fsbasis.T @ _vec_F(E)                         # MATLAB: temp = Fsbasis' * E(:)
        grad = -G + mu * _mat_F(Fsbasis @ temp, p)           # MATLAB: grad = -G + mu * reshape(Fsbasis*temp,d,d)
        E = E - (1.0 / L) * grad                             # MATLAB: E = E - (1/L)*grad   (gradient step)

        tnew = 0.5 * (1.0 + np.sqrt(1.0 + 4.0 * t * t))      # MATLAB: tnew = .5*(1+sqrt(1+4*t*t))

        Es = (E + E.T) / 2.0
        w, U = np.linalg.eigh(Es)                            # MATLAB: [u,e] = eig((E+E')/2)
        U = np.real(U)                                       # MATLAB: u = real(u)
        w = np.real(w)                                       # MATLAB: e = real(diag(e))
        eproj = proj_simplex(w)                              # MATLAB: eproj = proj_simplex(e(:))
        Dnew = (U * eproj) @ U.T                             # MATLAB: Dnew = u * diag(eproj) * u'

        D = (D + D.T) / 2.0                                  # MATLAB: D = (D+D')/2  (symmetrise previous D)
        E = Dnew + ((t - 1.0) / tnew) * (Dnew - D)           # MATLAB: E = Dnew + (t-1)/tnew * (Dnew - D)
        D = Dnew                                             # MATLAB: D = Dnew
        t = tnew                                             # MATLAB: t = tnew

        if it % 10 == 0:                                     # MATLAB: if mod(iter,10)==1
            temp = Fsbasis.T @ _vec_F(D)                     # MATLAB: temp = Fsbasis' * D(:)
            grad = -G + mu * _mat_F(Fsbasis @ temp, p)       # MATLAB: grad = -G + mu*reshape(Fsbasis*temp,d,d)
            # primal value:  -<G, D> + (mu/2) ||Π_F vec(D)||²
            primal_vals[it] = -float(np.sum(G * D)) + 0.5 * mu * float(temp @ temp)
                                                             # MATLAB: -sum(G(:).*D(:)) + mu/2*sum(temp.^2)
            # dual value:   λ_min(grad) - (mu/2) ||Π_F vec(D)||²
            dual_vals[it] = float(np.linalg.eigvalsh((grad + grad.T) / 2.0).min()) \
                - 0.5 * mu * float(temp @ temp)
                                                             # MATLAB: min(real(eig(grad))) - (mu/2)*sum(temp.^2)

            best_dual = dual_vals[: it + 1 : 10].max()       # MATLAB: max(dual_vals(1:10:iter))
            if (primal_vals[it] - best_dual) < tolerance:    # MATLAB: if (primal - max dual) < tolerance
                break                                        # MATLAB:   break  (duality gap satisfied)
...
def majorize_minimize(
    ...
    *,
    ...
    a candidate atom.
    """
    p = int(round(np.sqrt(Fs.shape[0])))                    # MATLAB: p = sqrt(size(Fs,1))
    Dinit = np.eye(p) / p                                    # MATLAB: Dinit = eye(p)/p  (PSD, trace 1)
    D = Dinit.copy()                                         # MATLAB: D = Dinit

    # Loop condition order is flipped vs. MATLAB: MATLAB checks ``norm(D)<1``
    # at the *top* of the while; we check it at the start of each iteration
    # which yields the same set of iterations because ``Dinit`` has Frobenius
    # norm 1/sqrt(p) < 1.
    for _ in range(nmmmax):                                  # MATLAB: while norm(D,'fro') < 1  (capped at nmmmax)
        if np.linalg.norm(D, "fro") >= 1.0:                  # MATLAB: while-condition
            break
        u, _ = extract_largest_eigenvector(G)                # MATLAB: u = extract_largest_eigenvector(G)
        Ginit = np.outer(u, u)                               # MATLAB: Ginit = u*u'   (rank-1 surrogate)
        D = solve_relaxation_mezcal_approx_fista(            # MATLAB: D = solve_relaxation_mezcal_approx_fista(...)
            Fs, Ginit, mu, Dinit, maxiter, tolerance
        )
        G = (D + D.T) / 2.0                                  # MATLAB: G = (D+D')/2   (refresh majorant)
    return D
...
def Ds_from_ds(ds: np.ndarray) -> np.ndarray:
    ...
    Ds = np.empty((p * p, k))                                # MATLAB: Ds = zeros(p^2, k)
    for i in range(k):                                       # MATLAB: for i = 1:k
        Ds[:, i] = _vec_F(np.outer(ds[:, i], ds[:, i]))      # MATLAB: Ds(:,i) = vec( ds(:,i)*ds(:,i)' )
    return Ds
...
def approx_ds_from_Ds(Ds: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    Ds = np.asarray(Ds, dtype=float)
    p2, k = Ds.shape                                         # MATLAB: k = size(Ds,2)
    p = int(round(np.sqrt(p2)))                              # MATLAB: p = sqrt(size(Ds,1))
    ds = np.empty((p, k))                                    # MATLAB: ds = zeros(p,k)
    eigmaxes = np.empty(k)                                   # MATLAB: eigmaxes = zeros(k,1)
    for i in range(k):                                       # MATLAB: for i = 1:k
        D = _mat_F(Ds[:, i], p)                              # MATLAB: D = reshape(Ds(:,i), p, p)
        u, e = extract_largest_eigenvector(D)                # MATLAB: [u,e] = extract_largest_eigenvector(D)
        eigmaxes[i] = e                                      # MATLAB: eigmaxes(i) = e
        ds[:, i] = u                                         # MATLAB: ds(:,i) = u
    return ds, eigmaxes
...
def adaptive_deflation(
    ...
    p = int(round(np.sqrt(Fs.shape[0])))                     # MATLAB: p = sqrt(size(Fs,1))
    if Ds_est is None:                                       # MATLAB: if nargin==2, Ds_est = []
        Ds_est = np.zeros((p * p, 0))
    Ds_est = np.asarray(Ds_est, dtype=float).reshape(p * p, -1)

    if Ds_est.shape[1] >= k:                                 # MATLAB: if size(Ds_est,2) < k  (else skip)
        return Ds_est

    kloc = k - Ds_est.shape[1]                               # MATLAB: kloc = k - size(Ds_est,2)
    Fsloc = np.hstack([Fs, Ds_est])                          # MATLAB: Fsloc = [Fs Ds_est]

    for i in range(kloc):                                    # MATLAB: for i = 1:kloc
        U, _, _ = np.linalg.svd(Fsloc, full_matrices=True)   # MATLAB: [uuu,~,~] = svd(Fsloc)
        ncols_to_keep = Fsloc.shape[1] - (kloc - i)          # MATLAB: end - (kloc-i+1)  (Python's i is 0-indexed)
        Fsloc_new = U[:, :ncols_to_keep]                     # MATLAB: Fsloc = uuu(:,1:end-(kloc-i+1))
        # MATLAB index range: end-(kloc-i):end -> (kloc-i+1) trailing columns
        Esloc = U[:, ncols_to_keep : ncols_to_keep + (kloc - i + 1)]   # MATLAB: Esloc = uuu(:,end-(kloc-i):end)

        G = _mat_F(Esloc[:, 0], p)                           # MATLAB: G = reshape(Esloc(:,1), p, p)
        D = majorize_minimize(G, Fsloc_new)                  # MATLAB: D = majorize_minimize(G, Fsloc)
        d_vec = _vec_F(D)[:, None]
        Ds_est = np.hstack([Ds_est, d_vec])                  # MATLAB: Ds_est = [Ds_est D(:)]
        Fsloc = np.hstack([Fsloc_new, d_vec])                # MATLAB: Fsloc = [Fsloc D(:)]
...
def _extract_clusters(DD: np.ndarray, nclust: int, ctype: str) -> np.ndarray:
    ...
    """
    p = int(round(np.sqrt(DD.shape[0])))                     # MATLAB: p = sqrt(size(DD,1))
    if ctype == "h":                                         # MATLAB: if strcmp(ctype,'h')
        # MATLAB ``clusterdata(X, nclust)`` defaults to single-linkage hierarchical
        # clustering with Euclidean distance and a max-cluster criterion.
        Z = linkage(DD.T, method="single", metric="euclidean")     # MATLAB: cc = clusterdata(DD', nclust)
        labels = fcluster(Z, t=nclust, criterion="maxclust")        # MATLAB: (cluster labels from above)
    elif ctype == "km":                                       # MATLAB: if strcmp(ctype,'km')
        km = KMeans(n_clusters=nclust, init="k-means++", n_init=10).fit(DD.T)  # MATLAB: cc = kmeans(DD, nclust)
        labels = km.labels_ + 1                              # (shift to MATLAB's 1-based labels)
    else:
        raise ValueError(f"unknown clustering type: {ctype!r}")

    Ds = np.zeros((p * p, nclust))                           # MATLAB: Ds_temp = zeros(p^2, nclust)
    unique_labels = np.unique(labels)
    for i, lbl in enumerate(unique_labels[:nclust]):         # MATLAB: for i = 1:nclust
        members = DD[:, labels == lbl]                       # MATLAB: DDi = DD(:, logical(cc==i))
        Ds[:, i] = members[:, 0]                             # MATLAB: Ds_temp(:,i) = DDi(:,1)
    return Ds
...
def cluster_Dss(Dss: np.ndarray, k: int, ctype: str = "h") -> np.ndarray:
    ...
    D1 = Dss[:, 0]                                           # MATLAB: D1 = Dss(:,1)  (reference direction)
    for i in range(1, Dss.shape[1]):                         # MATLAB: for i = 2:size(Dss,2)
        Dss[:, i] = np.sign(D1 @ Dss[:, i]) * Dss[:, i]      # MATLAB: Dss(:,i) = sign(D1'*Di) * Di
    return _extract_clusters(Dss, k, ctype)                  # MATLAB: extract_clusters(Dss, k, ctype)
...
def sdp_cluster(
    ...
    """
    ctype = _validate_ctype(ctype)                           # MATLAB: nargin/ctype validation block
    rng = _as_rng(rng)

    p = int(round(np.sqrt(Hs.shape[0])))                     # MATLAB: p = sqrt(size(Hs,1))
    nclust = 3 * k                                           # MATLAB: nclust = 3*k  (oversample 3×)

    _, Fs = extract_basis(Hs, k)                             # MATLAB: [~, Fs] = extract_basis(Hs, k)
    Dss = np.zeros((p * p, nclust))                          # MATLAB: Dss = zeros(p^2, nclust)
    for irep in range(nclust):                               # MATLAB: for irep = 1:nclust
        u = rng.standard_normal(p)                           # MATLAB: u = randn(p,1)
        u = u / np.linalg.norm(u)                            # MATLAB: u = u/norm(u)
        G = np.outer(u, u)                                   # MATLAB: G = u*u'   (rank-1 surrogate)
        D = majorize_minimize(G, Fs)                         # MATLAB: D = majorize_minimize(G, Fs)
        Dss[:, irep] = _vec_F(D)                             # MATLAB: Dss(:,irep) = D(:)

    Ds_est = cluster_Dss(Dss, k, ctype)                      # MATLAB: Ds_est = cluster_Dss(Dss, k, ctype)
    ds_est, _ = approx_ds_from_Ds(Ds_est)                    # MATLAB: ds_est = approx_ds_from_Ds(Ds_est)
...
def sdp_adaptive(Hs: np.ndarray, k: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    _, Fs = extract_basis(Hs, k)                             # MATLAB: [~, Fs] = extract_basis(Hs, k)
    Ds_est = adaptive_deflation(Fs, k)                       # MATLAB: Ds_est = adaptive_deflation(Fs, k)
    ds_est, _ = approx_ds_from_Ds(Ds_est)                    # MATLAB: ds_est = approx_ds_from_Ds(Ds_est)
    return ds_est, Ds_est
...
def sdp_semiada(
    ...
    """
    ctype = _validate_ctype(ctype)                           # MATLAB: nargin/ctype validation block
    _, Ds_clust = sdp_cluster(Hs, k, ctype, rng=rng)         # MATLAB: [~, Ds_clust] = sdp_cluster(Hs, k, ctype)

    G = np.abs(Ds_clust.T @ Ds_clust) - np.eye(k)            # MATLAB: G = abs(Ds_clust'*Ds_clust) - eye(k)
    mind = int(np.argmin(G.max(axis=0)))                     # MATLAB: [~, mind] = min(max(G))   (most isolated atom)
    selected = [Ds_clust[:, mind]]                           # MATLAB: Ds_est(:,1) = Ds_clust(:,mind)
    for i in range(k):                                       # MATLAB: for i = setdiff(1:k, mind)
        if i == mind:
            continue
        if G[:, i].max() < 0.8:                              # MATLAB: if max( G(:,i) ) < .8
            selected.append(Ds_clust[:, i])                  # MATLAB:   Ds_est(:,ind) = Ds_clust(:,i)
    Ds_est = np.column_stack(selected) if selected else np.zeros((Ds_clust.shape[0], 0))

    _, Fs = extract_basis(Hs, k)                             # MATLAB: [~, Fs] = extract_basis(Hs, k)
    Ds_est = adaptive_deflation(Fs, k, Ds_est)               # MATLAB: Ds_est = adaptive_deflation(Fs, k, Ds_est)
    ds_est, _ = approx_ds_from_Ds(Ds_est)                    # MATLAB: ds_est = approx_ds_from_Ds(Ds_est)
    return ds_est, Ds_est
